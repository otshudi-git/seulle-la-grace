...
export default function ReportsModule({ profile }: ReportsModuleProps) {
  ...
  const handleExportExcel = async () => {
    try {
      const { data: commandes } = await supabase
        .from('commandes')
        .select('numero_commande, date_commande, statut_livraison, statut_paiement, montant_total, montant_paye, montant_restant, client:clients(nom)');

      if (!commandes) return;

      const exportData = commandes.map((cmd: any) => ({
        'Numéro': cmd.numero_commande,
        'Date': new Date(cmd.date_commande).toLocaleDateString('fr-FR'),
        'Client': cmd.client?.nom || 'N/A',
        'Statut Livraison': cmd.statut_livraison,
        'Statut Paiement': cmd.statut_paiement,
        'Montant Total (USD)': cmd.montant_total,
        'Montant Payé (USD)': cmd.montant_paye,
        'Montant Restant (USD)': cmd.montant_restant,
      }));

      exportToExcel(exportData, 'rapport_general', 'Rapport');
    } catch (error) {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const headers = ['Numéro', 'Date', 'Client', 'Livraison', 'Paiement', 'Total', 'Payé', 'Restant'];
      const data = commandes.map((cmd: any) => [
        cmd.numero_commande,
        new Date(cmd.date_commande).toLocaleDateString('fr-FR'),
        cmd.client?.nom || 'N/A',
        cmd.statut_livraison,
        cmd.statut_paiement,
        `${cmd.montant_total} USD`,
        `${cmd.montant_paye} USD`,
        `${cmd.montant_restant} USD`,
      ]);

      exportToPDF('Rapport Général des Commandes', headers, data, 'rapport_general');
    } catch (error) {
