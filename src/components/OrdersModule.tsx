...
export default function OrdersModule({ profile }: OrdersModuleProps) {
  ...
  const [printReceiptType, setPrintReceiptType] = useState<'commande' | 'paiement'>('commande');
  ...
  const handlePrintCommande = (commande: Commande) => {
    const data = {
      ...
      client: {
        nom: commande.client?.nom || 'N/A',
        adresse: commande.client?.adresse,
        telephone: commande.client?.telephone,
      },
      items: commandeItems.map(item => ({
        nom: item.produit?.nom || 'N/A',
        quantite: item.quantite,
        prix_unitaire: item.prix_unitaire,
    ...
    setPrintReceiptType('commande');
    setShowPrintReceipt(true);
  };

  const handlePrintPaiement = (paiement: Paiement) => {
    if (!selectedCommande) return;

    const data = {
      numero: `PAY-${paiement.id.slice(0, 8)}`,
      date: paiement.date_paiement,
      client: {
        nom: selectedCommande.client?.nom || 'N/A',
        adresse: selectedCommande.client?.adresse,
        telephone: selectedCommande.client?.telephone,
      },
      montant_paye: paiement.montant,
      mode_paiement: paiement.mode_paiement,
    };
    setPrintReceiptData(data);
    setPrintReceiptType('paiement');
    setShowPrintReceipt(true);
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border-t border-gray-200 pt-4 flex justify-end">
                <button
                  onClick={() => handlePrintCommande(selectedCommande)}
      ...
      {showPrintReceipt && printReceiptData && (
        <PrintReceipt
